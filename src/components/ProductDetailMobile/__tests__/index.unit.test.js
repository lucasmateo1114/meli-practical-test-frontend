...
import ProductDetailMobile from "./../"
...
describe("ProductDetailMobile tests", () => {
  it("Should render correctly", () => {
    const { getByText } = render(
      <CurrenciesProvider>
        <ProductDetailMobile product={mockProduct} />
      </CurrenciesProvider>,
    )
  ...
  it("Should render correctly when sold quantity is greater than 0", () => {
    const { getByText } = render(
      <CurrenciesProvider>
        <ProductDetailMobile product={{ ...mockProduct, sold_quantity: 3 }} />
      </CurrenciesProvider>,
    )
